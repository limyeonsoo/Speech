...
for root, directory, files in os.walk('/home/knlab/Summer_study/Lab_members/JYC/honk/keyword_spotting_data_generator/data/'):
    for file in tqdm(files):
        if os.path.isfile(os.path.join(root,file).replace("wav","flac")):
            continue
        label = root.split('/')[-1]
        subprocess.call(cmd+os.path.join(root, file)+' '+"/mnt/sdb/YS/youtube_data/",shell=True)
