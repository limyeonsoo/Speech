...
URL_TEMPLATE = "https://youtube.com/watch?v={}"
...
ALIGNER_CONFIG_STRING = "task_language=eng|is_text_type=plain|os_task_file_format=json" # eng
...
def retrieve_keyword_audio(vid, keyword):
    ...
    print("Request URL:", v_url)
    youtube = YouTube(v_url)
    #youtube.player_config_args.get('player_response').get('videoDetails').get('length_seconds')
    ...
    if int(y_len) > 2700:
        ...
        return audio_index
    print("audio index:", audio_index)
    caption = youtube.captions.get_by_language_code('ko') #'en')
    ...
    if caption:
        ...
        for captions in cc_arr:
            ...
            words = Okt.nouns(cc_text) #cc_text.strip().split()
                    ...
                    if len(keyword) >= 3:
                        ...
                        for fragment in aligner_task.sync_map_leaves():
                            if fragment.is_regular and keyword in fragment.text and fragment.length < 0.9:
                                ...
                                print( DATA_DIR + "/" + keyword + "/" + file_name)
